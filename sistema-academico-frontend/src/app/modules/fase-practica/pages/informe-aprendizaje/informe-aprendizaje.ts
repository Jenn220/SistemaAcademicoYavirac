...
export class InformeAprendizaje implements OnInit {
  ...
  private aplicarReaperturaComoBorrador(): void {
    ...
    Swal.fire({
      ...
      text: 'Este informe ya estaba aprobado; al guardar cambios nuevos vuelve a borrador y debe enviarse a revisión otra vez.',
      timer: 4500,
        ...
        this.documentos.actualizarEstadoDocumento(this.idDocumento!, 'pendiente_revision', '').subscribe({
          ...
          error: () => {
            this.verificarEstadoTrasError(
              'pendiente_revision',
              'El informe se envió a revisión correctamente.',
              'No fue posible enviar el informe a revisión.'
            );
          },
        });
      }
      ...
      if (result.isConfirmed) {
        this.documentos.actualizarEstadoDocumento(this.idDocumento!, 'aprobado').subscribe({
          ...
          error: () => {
            this.verificarEstadoTrasError(
              'aprobado',
              'El informe fue aprobado correctamente.',
              'No fue posible aprobar el informe.'
            );
          },
        });
      }
