...
export class EvaluacionInstituto implements OnInit {
   ...
   * formen un número válido (ej. "-" repetido produce "1-1-1" visible en el
   * campo, aunque el modelo nunca lo reciba). Solo se permiten dígitos, un
   * único punto decimal y las teclas de edición/navegación.
   */
   ...
   * "aprobado" con cambios que el coordinador/estudiante nunca llegaron a
   * revisar. No se pasa en true en la carga inicial de la página, solo justo
   * después de guardar.
   */
  ...
  /**
   * Reabre a "borrador" un documento que ya estaba aprobado, para que vuelva
   * a pasar por revisión tras guardar cambios nuevos. Usa el mismo endpoint
   * de "cambiar estado" que enviarARevision/aprobar, así que también puede
   * toparse con el mismo bug del backend (aplica el cambio pero responde
   * error) — por eso se relee el estado real ante un error en vez de asumir
   * que falló.
   */
  ...
  private aplicarReaperturaComoBorrador(): void {
    ...
    Swal.fire({
      ...
      text: 'Esta evaluación ya estaba aprobada; al guardar cambios nuevos vuelve a borrador y debe enviarse a revisión otra vez.',
      timer: 4500,
