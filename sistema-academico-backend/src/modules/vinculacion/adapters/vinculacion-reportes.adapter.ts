...
async obtainActaCompromisoRaw(idVinculacion: number): Promise<any> {
  const query = `
    SELECT 
      ...
      -- 👇 Prioriza la Entidad Receptora sobre la Empresa genérica
      COALESCE(er.nombre_entidad, emp.razon_social, 'Sin Entidad Asignada') AS entidad_beneficiaria,
    ...
    -- 👇 LEFT JOINs para evitar que la consulta falle si id_empresa o id_entidad_receptora es nulo
    LEFT JOIN empresa emp ON vinc.id_empresa = emp.id_empresa
    LEFT JOIN vinculacion_entidad_receptora er ON vinc.id_entidad_receptora = er.id_entidad
...
async obtenerAsistenciaEstudianteRaw(idVinculacion: number): Promise<any[]> {
  const query = `
    SELECT 
      ...
      -- 👇 Traemos directamente el tutor de la entidad receptora
      er.tutor_entidad_receptora AS tutor_entidad,
      
      per.nombre AS periodo_academico,
      vinc.total_horas_estudiante,
      
      -- OBSERVACIÓN DINÁMICA DEL ESTUDIANTE
      (
        SELECT observacion 
    ...
    -- 👇 Cambiado a LEFT JOIN e integrada la entidad receptora
    LEFT JOIN empresa emp ON vinc.id_empresa = emp.id_empresa
    LEFT JOIN vinculacion_entidad_receptora er ON vinc.id_entidad_receptora = er.id_entidad
...
async obtainReporteAsistenciaTutorRaw(idVinculacion: number): Promise<any[]> {
  const query = `
    SELECT 
      ...
      -- 👇 Evita espacios en blanco si los nombres vienen vacíos/nulos
      COALESCE(
        NULLIF(TRIM(CONCAT(coord.nombres, ' ', coord.apellidos)), ''), 
        'Sin Coordinador Asignado'
      ) AS coordinador_carrera,
      
      -- OBSERVACIÓN DINÁMICA DEL TUTOR
      (
        SELECT observacion 
        FROM vinculacion_reporte_observacion 
        WHERE id_vinculacion = vinc.id_vinculacion 
    ...
    -- 👇 Cambiado a LEFT JOIN e integrada la nueva tabla de entidad receptora
    LEFT JOIN empresa emp ON vinc.id_empresa = emp.id_empresa
    LEFT JOIN vinculacion_entidad_receptora er ON vinc.id_entidad_receptora = er.id_entidad

    -- Unión con periodo_carrera para identificar al coordinador
    LEFT JOIN periodo_carrera pc ON pc.id_periodo = vinc.id_periodo AND pc.id_carrera = m.id_carrera
    LEFT JOIN docente coord ON pc.id_coordinador = coord.id_docente
...
async obtainInformeFinalRaw(idVinculacion: number): Promise<any[]> {
  const query = `
    SELECT 
      ...
      -- 👇 SI EXISTE ENTIDAD RECEPTORA USA SUS DATOS, SINO USA EMPRESA
    ...
    -- 👇 AQUÍ UNIMOS LA NUEVA TABLA DE ENTIDADES RECEPTORAS
    LEFT JOIN vinculacion_entidad_receptora er ON vinc.id_entidad_receptora = er.id_entidad
